...
| B10 Medium backend — partially completed | Direct-KMS plaintext-size boundary completed with REVIEW_PASS; pagination overflow/ties and dependency error behavior remain unresolved | Preserve the reviewed [application/data contract](architecture.md#current-contract--application-and-data) and [validation coverage](validation.md#existing-test-meaning-and-gaps); larger-payload support remains a separate future design decision | [Text-size boundary](../../specs/kms-text-boundary/spec.md) complete; remaining correctness gaps are separate work |
